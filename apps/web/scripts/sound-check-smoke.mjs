...
function emitAck(socket, event, payload) {
  return new Promise((resolve, reject) => {
    socket.emit(event, payload, (response) => response?.ok ? resolve(response.data) : reject(new Error(response?.error ?? `${event} failed`)));
  });
}
...
async function createSoundCheckRoom() {
  ...
  if (blueprint.releaseStatus !== "release_ready") throw new Error("SoundCheck blueprint did not pass the release gate");
...
const clients = ["Avery", "Blake", "Casey", "Drew"].map((name) => ({ name, socket: io(apiUrl, { transports: ["websocket"] }), playerId: "", view: null }));
...
try {
  ...
  for (let index = 0; index < clients.length; index += 1) {
    ...
    await emitAck(client.socket, "room:team:select", { code: room.code, playerId: client.playerId, teamId: teamIds[index % 2] });
  }
  await emitAck(clients[0].socket, "room:captain:select", { code: room.code, playerId: clients[0].playerId, teamId: teamIds[0], captainPlayerId: clients[0].playerId });
  await emitAck(clients[0].socket, "room:captain:select", { code: room.code, playerId: clients[0].playerId, teamId: teamIds[1], captainPlayerId: clients[1].playerId });
  ...
  const performerSelected = nextState(performer, (view) => view.kind === "composed" && view.phase.id === "draw_sound" && view.activePlayerId === performerId);
  await gameAction(captain, room.code, { type: "COMPOSED_ACTION", actionId: "select_performer", payload: { targetPlayerId: performerId } });
  await performerSelected;

  const liveViews = clients.map((client) => nextState(client, (view) => view.kind === "composed" && view.phase.id === "performing"));
  ...
  if (audience.some((client) => JSON.stringify(client.view).includes(secret))) throw new Error("A private SoundCheck prompt leaked to the audience");
  const guesser = audience.find((client) => client.view.availableActions.some((action) => action.id === "submit_guess"));
  if (!guesser) throw new Error("No eligible SoundCheck guesser was found");

  await gameAction(guesser, room.code, { type: "COMPOSED_ACTION", actionId: "submit_guess", payload: { text: "Definitely wrong" } });
  ...
  await gameAction(guesser, room.code, { type: "COMPOSED_ACTION", actionId: "submit_guess", payload: { text: secret.toUpperCase() } });
  await nextState(guesser, (view) => view.kind === "composed" && view.phase.id === "select_performer");
  if ((guesser.view.scores.teams[scoringTeamId] ?? 0) !== scoreBefore + 1) throw new Error("The exact SoundCheck guess did not score");

  console.log(JSON.stringify({ room: room.code, game: "SoundCheck", teams: teamIds.length, privatePromptProtected: true, wrongGuessRejected: true, exactGuessScored: true, revision: guesser.view.revision }));
} finally {
