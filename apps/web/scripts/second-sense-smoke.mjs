...
function nextState(client, predicate) {
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      ...
      reject(new Error("Timed out waiting for Second Sense state"));
    }, 6_000);
    const receive = (view) => {
...
async function createRoom() {
  const blueprintResponse = await fetch(`${apiUrl}/api/second-sense/blueprints`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ themeId: "cyberpunk", tempo: "quickfire" }),
  });
  ...
  if (blueprint.releaseStatus !== "release_ready" || blueprint.playtest.completedSimulations !== 24)
    throw new Error("Second Sense did not pass 24 virtual playtests");
...
async function attempt(client, clients, code, offsetMs) {
  ...
  await actAndSync(client, clients, code, {
...
const clients = ["Maya", "Noah", "Avery", "Kai", "Zoe"].map((name) => ({
  name,
...
try {
  ...
  if (privateWaitingBoard.lastRound !== null || privateWaitingBoard.ownAttemptMs !== undefined)
    throw new Error("A timing result leaked before the reveal");
  ...
  if (board(clients[0]).lastRound.qualifiedPlayerIds.length !== 3)
    throw new Error("The closest half did not survive stage one");
  if (board(clients[0]).lastRound.entries.some((entry) => entry.elapsedMs === 30_000))
    throw new Error("The server trusted a forged client duration");
  ...
  const semifinalists = board(clients[0]).activePlayerIds.map((playerId) =>
    clients.find((client) => client.playerId === playerId),
  );
  ...
  if (board(clients[0]).lastRound.qualifiedPlayerIds.length !== 2)
    throw new Error("The semifinal did not produce two finalists");

  await actAndSync(clients[0], clients, room.code, { type: "COMPOSED_ACTION", actionId: "next_stage" });
  const finalists = board(clients[0]).activePlayerIds.map((playerId) =>
    clients.find((client) => client.playerId === playerId),
  );
  ...
  if (clients[0].view.winner?.ids[0] !== finalists[0].playerId)
    throw new Error("The closest finalist was not declared the winner");
  ...
  console.log(
    JSON.stringify({
      ...
      privateAttemptsProtected: true,
      serverAuthoritativeTiming: true,
      eliminationStages: 3,
