...
async function createRoom() {
  const blueprintResponse = await fetch(`${apiUrl}/api/story-chain/blueprints`, {
    ...
    body: JSON.stringify({ themeId: "cozy", mood: "mystery", length: "quick", preferences: "A birthday mystery at an old hotel" }),
  });
  ...
  return { ...(await roomResponse.json()), title: blueprint.title, simulations: blueprint.playtest.completedSimulations };
}
...
try {
  ...
  for (let chapter = 1; chapter <= 8; chapter += 1) {
    ...
    const writingViews = clients.map((client) => nextState(client, (view) => view.kind === "composed" && view.phase.id === "write" && view.round === chapter));
    ...
    if (readers.some((client) => JSON.stringify(client.view).includes(secret))) throw new Error(`Private twist leaked in chapter ${chapter}`);

    if (chapter === 1) {
      try {
        await gameAction(author, room.code, { type: "COMPOSED_ACTION", actionId: "continue_story", payload: { text: "A perfectly ordinary sentence followed." } });
      } catch (error) {
    ...
    const resolvedViews = clients.map((client) => nextState(client, (view) => view.kind === "composed" && view.revision >= nextRevision));
    await gameAction(author, room.code, { type: "COMPOSED_ACTION", actionId: "continue_story", payload: { text: contribution } });
    await Promise.all(resolvedViews);
  }

  if (!clients.every((client) => client.view.status === "completed")) throw new Error("StoryChain did not complete after eight chapters");
  const finalStory = clients[0].view.components.find((component) => component.kind === "story")?.data;
  if (!Array.isArray(finalStory?.entries) || finalStory.entries.length !== 8) throw new Error("The final manuscript does not contain eight chapters");
  console.log(JSON.stringify({ room: room.code, game: "StoryChain", title: room.title, players: clients.length, chapters: finalStory.entries.length, privateTwistsProtected: true, missingWordRejected: rejectedMissingWord, virtualPlaytests: room.simulations }));
} finally {
