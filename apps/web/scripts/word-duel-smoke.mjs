...
function nextState(client, predicate) {
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      ...
      reject(new Error("Timed out waiting for WordDuel state"));
    }, 6_000);
    const receive = (view) => {
...
async function createRoom() {
  const blueprintResponse = await fetch(`${apiUrl}/api/word-duel/blueprints`, {
    ...
    body: JSON.stringify({ themeId: "minimal", difficulty: "classic" }),
  });
  if (!blueprintResponse.ok) throw new Error(`Blueprint creation failed: ${blueprintResponse.status}`);
  const blueprint = await blueprintResponse.json();
  if (blueprint.releaseStatus !== "release_ready" || blueprint.playtest.completedSimulations !== 24)
    throw new Error("WordDuel did not pass 24 virtual playtests");
...
async function actAndSync(actor, clients, code, action) {
  ...
  const updates = clients.map((client) =>
    nextState(client, (view) => view.kind === "composed" && view.revision >= revision),
  );
...
try {
  ...
  await actAndSync(clients[0], clients, room.code, {
    type: "COMPOSED_ACTION",
    actionId: "lock_word",
    payload: { text: words.get(clients[0].playerId) },
  });
  await actAndSync(clients[1], clients, room.code, {
    type: "COMPOSED_ACTION",
    actionId: "lock_word",
    payload: { text: words.get(clients[1].playerId) },
  });
  if (!clients.every((client) => client.view.phase.id === "duel"))
    throw new Error("The duel did not begin after both words were locked");
  for (const client of clients) {
    const opponent = clients.find((candidate) => candidate !== client);
    if (JSON.stringify(client.view).includes(words.get(opponent.playerId)))
      throw new Error("An opponent secret word leaked before play");
  }
  ...
  await actAndSync(first, clients, room.code, {
    type: "COMPOSED_ACTION",
    actionId: "guess_letter",
    payload: { text: correctLetter },
  });
  const firstBoard = first.view.components.find((component) => component.kind === "word_duel")?.data;
  if (!firstBoard.opponentMask.includes(correctLetter))
    throw new Error("A correct letter was not revealed in the word");
  if (firstBoard.keyboard.find((key) => key.letter === correctLetter)?.state !== "correct")
    throw new Error("A winning keyboard key was not disabled as correct");

  const secondTarget = words.get(first.playerId);
  const miss = [..."QZXJV"].find((letter) => !secondTarget.includes(letter));
  await actAndSync(second, clients, room.code, {
    type: "COMPOSED_ACTION",
    actionId: "guess_letter",
    payload: { text: miss },
  });
  const secondBoard = second.view.components.find((component) => component.kind === "word_duel")?.data;
  if (secondBoard.keyboard.find((key) => key.letter === miss)?.state !== "wrong")
    throw new Error("A missed keyboard key was not disabled");

  let repeatedRejected = false;
  try {
    await gameAction(first, room.code, {
      type: "COMPOSED_ACTION",
      actionId: "guess_letter",
      payload: { text: correctLetter },
    });
  } catch (error) {
    repeatedRejected = error instanceof Error && error.message.includes("already been played");
  }
  ...
  if (!clients.every((client) => client.view.status === "completed"))
    throw new Error("The correct full word did not end the duel");
  ...
  console.log(
    JSON.stringify({
      ...
      game: "WordDuel",
      players: 2,
      privateWordsProtected: true,
      correctKeyDisabled: true,
      wrongKeyDisabled: true,
      repeatedLetterRejected: repeatedRejected,
      fullWordSolved: true,
