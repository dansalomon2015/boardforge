...
export default function RoomPage() {
  ...
  const isOriginal =
    isMovieMime || isWordTrap || isDrawBattle || isSoundCheck || isStoryChain || isWordDuel || isSecondSense;

  return (
    <main
      className={`room-shell ${isOriginal ? roomChromeStyles.movieRoom : ""}`}
      data-original-game={
        isSecondSense
          ? "second-sense"
          : isWordDuel
          ...
          <div className="lobby-hero">
                ...
                : isWordDuel
                  ...
                  : isStoryChain
                    ...
                    : isSoundCheck
                      ...
                      : isDrawBattle
                        ? "The gallery is opening"
                        : isWordTrap
                          ? "Teams are entering the trap"
                          : isMovieMime
                            ? "Casting in progress"
            ...
            {isOriginal ? (
              <div className={roomChromeStyles.lobbyFacts}>
                {isSecondSense ? (
                  <>
                    <span>◉ Invisible clock</span>
                    <span>⌁ Knockout rounds</span>
                    <span>✦ 2–12 players</span>
                  </>
                ) : isWordDuel ? (
                  <>
                    <span>W Secret words</span>
                    <span>⌨ Playable keyboard</span>
                    <span>⚔ Exactly 2 players</span>
                  </>
                ) : isStoryChain ? (
                  <>
                    <span>✦ One shared story</span>
                    <span>⌁ Secret twists</span>
                    <span>♡ No teams, no score</span>
                  </>
                ) : (
                  <>
                    <span>
                    ...
                    <span>⏱ {isDrawBattle ? "75" : "60"} seconds</span>
                    <span>✦ {view.teamSetup?.teams.length ?? 2} teams</span>
                  </>
                )}
              </div>
            ) : null}
            ...
            {self?.isHost ? (
              ...
              >
                {view.canStart
                  ? "Start the game"
                  : isSecondSense
                    ? "Invite at least one timekeeper"
                    : isWordDuel
                      ? "Invite your rival"
                      : isStoryChain
                        ? "Invite one more storyteller"
                        : (view.startBlockReason ?? "The room is not ready yet")}{" "}
                <b>→</b>
              </button>
            ) : (
