  ...
  async function prepareGame(event: FormEvent) {
    ...
    try {
      const blueprintResponse = await fetch(`${apiUrl}/api/second-sense/blueprints`, {
        ...
        body: JSON.stringify({ themeId: theme, tempo }),
      });
      ...
      if (prepared.releaseStatus !== "release_ready")
        throw new Error("This arena needs another rules check. Try again.");
    ...
    <main className={`${baseStyles.page} ${styles.page}`}>
      ...
      <form className={baseStyles.layout} onSubmit={prepareGame}>
        <section className={baseStyles.configuration}>
          <div className={`${baseStyles.titleBlock} ${styles.titleBlock}`}>
            <p>BoardForge Original No. 07</p>
            <h1>
              Stop time.
              <br />
            ...
            <span>
              See the target. Touch once to begin, once to stop. The clock disappears — and the closest players survive.
            </span>
          </div>
          ...
          <fieldset className={baseStyles.fieldset}>
            <legend>
              <b>01</b>
              <span>Choose the tempo</span>
              <small>Targets change every elimination round</small>
            </legend>
            <div className={styles.tempoGrid}>
              {tempos.map((item) => (
                <button
                  className={tempo === item.id ? styles.selected : ""}
                  key={item.id}
          ...
          <fieldset className={baseStyles.fieldset}>
            <legend>
              ...
              <small>Every pulse takes on your colors</small>
            </legend>
          ...
          <section className={styles.rules}>
            <p>One rule. Pure instinct.</p>
            <div>
              <span>01</span>
              <strong>Memorize the target</strong>
              <small>The number disappears the moment you begin.</small>
            </div>
            <div>
              <span>02</span>
              <strong>Stop when it feels right</strong>
              <small>No clock, progress ring, or repeating visual cue.</small>
            </div>
            <div>
              <span>03</span>
              <strong>Survive the cut</strong>
              <small>The closest half advances until one player remains.</small>
            </div>
          </section>
          {error ? (
          ...
          <button className={`${baseStyles.submit} ${styles.submit}`} disabled={busy} type="submit">
            <span>{busy ? "Calibrating the pulse…" : "Open the Second Sense room"}</span>
            <b>{busy ? "•••" : "↗"}</b>
          </button>
          <p className={baseStyles.submitNote}>
            Bring 2–12 players. No teams, no player-count setup, no visible countdown.
          </p>
        </section>
          ...
          <div className={baseStyles.sticky}>
            ...
            <div className={styles.preview} style={previewStyle}>
              ...
              <div className={styles.previewCopy}>
                <small>Internal clock elimination</small>
                <h2>
                  Second
                  <br />
                  <em>Sense</em>
              ...
              <div className={styles.previewCut}>
                <span>8 players</span>
                <i />
                <span>4 survive</span>
                <i />
                <span>2 finalists</span>
              </div>
              <div className={styles.previewFooter}>
                <span>
                  <b>{activeTempo.range}</b> target range
                </span>
                <span>
                  <b>2–12</b> players
                </span>
      ...
      {busy ? (
        <div className={`${baseStyles.loading} ${styles.loading}`} aria-live="polite">
          ...
          <p>Calibrating your internal clock…</p>
          <small>Drawing fresh targets · testing the cut · opening the room</small>
        </div>
      ) : null}
    </main>
  );
