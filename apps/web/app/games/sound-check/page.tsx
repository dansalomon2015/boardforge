  ...
  async function prepareGame(event: FormEvent) {
    ...
    try {
      ...
      if (prepared.releaseStatus !== "release_ready")
        throw new Error("That sound mix needs a quick rethink. Try another theme.");
    ...
    <main className={styles.page}>
      ...
      <form className={styles.layout} onSubmit={prepareGame}>
        <section className={styles.configuration}>
          <div className={styles.titleBlock}>
            <p>BoardForge Original No. 04</p>
            <h1>
              Make tonight&apos;s
              <br />
              <em>SoundCheck.</em>
            </h1>
            <span>Set the mood, name your teams, and choose the sounds most likely to bring the house down.</span>
          </div>
            ...
            <div className={styles.countGrid}>
              {promptCounts.map((count) => (
                ...
                >
                  <strong>{count}</strong>
                  <span>sounds</span>
                  <small>~{Math.ceil(count * 1.1)} min</small>
                </button>
              ))}
            </div>
          </fieldset>
          ...
          <fieldset className={styles.fieldset}>
            <legend>
              <b>04</b>
              <span>Make the sounds yours</span>
              <small>Optional · leave it blank for a great surprise</small>
            </legend>
            <textarea
              maxLength={240}
              onChange={(event) => setPreferences(event.target.value)}
              placeholder="e.g. Easy animals and household sounds for a family game night…"
              rows={4}
              value={preferences}
            />
            <div className={styles.suggestions}>
              {["Easy animals", "Machines only", "Music studio", "Hard mode"].map((suggestion) => (
          ...
          <button className={styles.submit} disabled={busy} type="submit">
            <span>{busy ? "Turning up the volume…" : "Open the SoundCheck room"}</span>
            <b>{busy ? "•••" : "↗"}</b>
          </button>
          <p className={styles.submitNote}>
            {preferences.trim()
              ? "Your brief will shape tonight’s sound mix."
          ...
          <div className={styles.sticky}>
            ...
            <div className={styles.gamePreview} style={previewStyle}>
              ...
              <div className={styles.previewCenter}>
                <small>Voice-only sound showdown</small>
                <h2>
                  Sound
                  <br />
                  Check
                ...
                <p>
                  {preferences.trim() ||
                    "Secret sounds, fearless voices, and a room racing to identify every performance."}
                </p>
              </div>
              <div className={styles.previewStats}>
                <span>
                  <b>{promptCount}</b> sounds
                </span>
                <span>
      ...
      {busy ? (
        <div className={styles.loading} aria-live="polite">
          ...
          <p>{preferences.trim() ? "Tonight’s set is taking shape…" : "The studio lights are coming on…"}</p>
          <small>Picking the sounds · setting the mood · opening the room</small>
        </div>
      ) : null}
    </main>
  );
