  ...
  async function prepareGame(event: FormEvent) {
    ...
    try {
      ...
      if (prepared.releaseStatus !== "release_ready")
        throw new Error("That story needs another draft. Try again for a fresh opening.");
    ...
    <main className={`${baseStyles.page} ${styles.page}`}>
      ...
      <form className={baseStyles.layout} onSubmit={prepareGame}>
        <section className={baseStyles.configuration}>
          <div className={baseStyles.titleBlock}>
            <p>BoardForge Original No. 05</p>
            <h1>
              Write a story
              <br />
              <em>no one expects.</em>
            </h1>
            <span>
              Set the atmosphere. We will create the opening and secret twists. Your group writes everything that
              happens next.
            </span>
          </div>
          ...
          <fieldset className={baseStyles.fieldset}>
            <legend>
              <b>01</b>
              <span>Choose the story mood</span>
              <small>What kind of tale are we telling?</small>
            </legend>
            <div className={styles.moodGrid}>
              {moods.map((item) => (
                <button
                  className={mood === item.id ? styles.selected : ""}
                  key={item.id}
          ...
          <fieldset className={baseStyles.fieldset}>
            <legend>
              <b>02</b>
              <span>Choose the edition</span>
              <small>The room adapts to whoever joins</small>
            </legend>
            <div className={styles.lengthGrid}>
              {lengths.map((item) => (
                <button
                  className={length === item.id ? styles.selected : ""}
                  key={item.id}
                  onClick={() => setLength(item.id)}
                  type="button"
                >
                  <span>{item.name}</span>
                  <strong>{item.chapters}</strong>
                  <small>chapters · {item.time}</small>
                </button>
              ))}
            </div>
          </fieldset>
          ...
          <fieldset className={baseStyles.fieldset}>
            <legend>
              <b>03</b>
              <span>Choose the visual world</span>
              <small>Your story, beautifully bound</small>
            </legend>
          ...
          <fieldset className={baseStyles.fieldset}>
            <legend>
              <b>04</b>
              <span>Give us a spark</span>
              <small>Optional · a sentence is plenty</small>
            </legend>
            <textarea
              maxLength={240}
              onChange={(event) => setPreferences(event.target.value)}
              placeholder="e.g. A wedding on a train where every guest is hiding something…"
              rows={4}
              value={preferences}
            />
            <div className={baseStyles.suggestions}>
              {["A hotel after midnight", "A magical family reunion", "Friends lost in time", "Surprise me"].map(
                (suggestion) => (
                  <button
                    key={suggestion}
                    onClick={() => setPreferences(suggestion === "Surprise me" ? "" : suggestion)}
                    type="button"
                  >
                    {suggestion}
                  </button>
          ...
          <button className={baseStyles.submit} disabled={busy} type="submit">
            <span>{busy ? "Writing the first page…" : "Open the StoryChain room"}</span>
            <b>{busy ? "•••" : "↗"}</b>
          </button>
          <p className={baseStyles.submitNote}>
            No teams. No score. Just one story your group could never write twice.
          </p>
        </section>
        ...
        <aside className={baseStyles.previewColumn}>
          <div className={baseStyles.sticky}>
            ...
            <div className={styles.book} style={previewStyle}>
              ...
              <div className={styles.bookTitle}>
                <small>{activeMood.name} collaborative fiction</small>
                <h2>
                  Story
                  <br />
                  <em>Chain</em>
                ...
                <p>
                  {preferences.trim() || "One opening. A secret twist for every writer. An ending nobody sees coming."}
                </p>
              </div>
              <div className={styles.bookFooter}>
                <span>{activeLength.chapters} chapters</span>
            ...
            <div className={baseStyles.previewDetails}>
              ...
              <div>
                <span>Edition</span>
                <b>{activeLength.name}</b>
              </div>
              <div>
                <span>Written by</span>
                <b>Everyone in the room</b>
              </div>
            </div>
          </div>
        </aside>
      ...
      {busy ? (
        <div className={`${baseStyles.loading} ${styles.loading}`} aria-live="polite">
          ...
          <p>Your opening is taking shape…</p>
          <small>Setting the scene · hiding the twists · testing every chapter</small>
        </div>
      ) : null}
    </main>
  );
