  ...
  async function prepareGame(event: FormEvent) {
    ...
    try {
      ...
      if (prepared.releaseStatus !== "release_ready")
        throw new Error("That word mix needs a quick rethink. Try another theme.");
    ...
    <main className={styles.page}>
      ...
      <form className={styles.layout} onSubmit={prepareGame}>
        <section className={styles.configuration}>
          <div className={styles.titleBlock}>
            ...
            <span>
              Set the mood, name your teams, and choose the kind of words that will keep everyone on their toes.
            </span>
          </div>
          ...
          <fieldset className={styles.fieldset}>
            ...
            <p className={styles.captainNote}>
              <span>★</span> Players join teams in the room. The host then appoints one captain per team.
            </p>
          </fieldset>
          ...
          <fieldset className={styles.fieldset}>
            ...
            <div className={styles.countGrid}>
              {cardCounts.map((count) => (
                <button
                  className={cardCount === count ? styles.selectedCount : ""}
                  key={count}
                ...
                >
                  <strong>{count}</strong>
                  <span>cards</span>
                  <small>~{Math.ceil(count * 1.15)} min</small>
                </button>
              ))}
            </div>
          </fieldset>
          ...
          <fieldset className={styles.fieldset}>
            ...
            <textarea
              ...
              placeholder="e.g. Easy pop culture, travel and food for a family game night…"
              rows={4}
              value={preferences}
            />
            <div className={styles.suggestions}>
              {["Pop culture", "Family night", "Food & travel", "Hard mode"].map((suggestion) => (
          ...
          <button className={styles.submit} disabled={busy} type="submit">
            <span>{busy ? "Setting the trap…" : "Open the WordTrap room"}</span>
            <b>{busy ? "•••" : "↗"}</b>
          </button>
          <p className={styles.submitNote}>
            {preferences.trim()
              ? "Your brief will shape tonight’s word mix."
          ...
          <div className={styles.sticky}>
            ...
            <div className={styles.gamePreview} style={previewStyle}>
              ...
              <div className={styles.previewCenter}>
                <small>Forbidden-word showdown</small>
                <h2>
                  Word
                  <br />
                  Trap
      ...
      {busy ? (
        <div className={styles.loading} aria-live="polite">
          ...
          <p>{preferences.trim() ? "Tonight’s word mix is taking shape…" : "The trap is being set…"}</p>
          <small>Picking the words · setting the mood · opening the room</small>
        </div>
      ) : null}
    </main>
  );
