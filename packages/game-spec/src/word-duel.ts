...
export const wordDuelSetupSchema = z
  .object({
...
export function createWordDuelSpec(setupInput: WordDuelSetupInput): ComposedGameSpec {
    ...
    components: [
      {
        ...
        title: { kind: "literal", value: "WordDuel" },
        description: { kind: "literal", value: "Two secret words. Twenty-six letters. One winner." },
        icon: "W",
      },
      ...
      { id: "players", kind: "players", audience: "public" },
      {
        ...
        kind: "outcome",
        audience: "public",
        title: { kind: "literal", value: "Word cracked" },
        description: { kind: "literal", value: "The winning word has been revealed." },
      },
    ],
    actions: [
      {
        ...
        actor: "all_players",
        oncePerPhase: true,
        wordDuelId: "duel_board",
        effects: [{ kind: "register_secret_word", wordDuelId: "duel_board" }],
      },
      {
        id: "guess_letter",
        label: "Play this letter",
        kind: "letter_guess",
        actor: "active_player",
        oncePerPhase: false,
        wordDuelId: "duel_board",
        effects: [
          { kind: "guess_letter", wordDuelId: "duel_board" },
          { kind: "set_active_player", mode: "next" },
        ],
      },
      {
        id: "solve_word",
        label: "Solve the word",
        kind: "word_guess",
        actor: "active_player",
        oncePerPhase: false,
        wordDuelId: "duel_board",
        effects: [
          { kind: "guess_word", wordDuelId: "duel_board" },
          { kind: "set_active_player", mode: "next" },
        ],
      },
    ],
    ...
    phases: [
      {
        ...
        componentIds: ["game_header", "duel_board", "players"],
        actionIds: ["lock_word"],
        nextPhaseId: "duel",
        completionMode: "all",
        completionConditions: [{ kind: "all_players_acted", actionId: "lock_word" }],
        onComplete: [],
      },
      {
        id: "duel",
        title: "The duel is live",
        componentIds: ["game_header", "duel_board", "turn", "players", "outcome"],
        actionIds: ["guess_letter", "solve_word"],
  ...
  if (!validation.ok)
    throw new Error(
      `Invalid WordDuel GameSpec: ${validation.issues.map((issue) => `${issue.code}:${issue.path}`).join(", ")}`,
    );
