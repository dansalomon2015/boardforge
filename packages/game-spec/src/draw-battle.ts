...
export const drawBattlePromptSchema = z.object({
...
export const drawBattleSetupSchema = z.object({
...
export const drawBattlePackSchema = z.object({
...
export function createDrawBattlePack(setupInput: DrawBattleSetupInput, promptIds: readonly string[], source: DrawBattlePack["source"]): DrawBattlePack {
  const setup = drawBattleSetupSchema.parse(setupInput);
  if (promptIds.length !== setup.promptCount) throw new Error(`DrawBattle requires exactly ${setup.promptCount} prompts.`);
  ...
  return drawBattlePackSchema.parse({ schemaVersion: 1, game: "draw_battle", source, themeId: setup.themeId, ...(setup.preferences ? { preferences: setup.preferences } : {}), teams: setup.teams.map((team, index) => ({ id: `team_${index + 1}`, ...team })), prompts });
}

export function createRandomDrawBattlePack(setupInput: DrawBattleSetupInput, seed: string): DrawBattlePack {
  const setup = drawBattleSetupSchema.parse(setupInput);
  return createDrawBattlePack(setup, deterministicShuffle(drawBattleCatalog, seed).slice(0, setup.promptCount).map((prompt) => prompt.id), "random");
}
...
export function createDrawBattleSpec(packInput: DrawBattlePack): ComposedGameSpec {
  ...
  const suffix = numberHash(`draw-battle-v1:${pack.themeId}:${pack.prompts.map((prompt) => prompt.id).join(":")}`).toString(36);
  ...
  const spec: ComposedGameSpec = {
    ...
    template: "composed",
    title: "DrawBattle",
    description: pack.preferences ? `A live drawing showdown tailored around ${pack.preferences}.` : "Draw together, race to name the secret prompt, and turn every line into points.",
    theme,
    minPlayers: pack.teams.length,
    maxPlayers: 12,
    suggestedDurationMinutes: Math.max(10, Math.ceil(rounds * 1.35)),
    setup: { mode: "teams", teamPolicy: { teams: pack.teams, minMembersPerTeam: 1, maxMembersPerTeam: 6, allocation: "balanced", allowUnevenTeams: true, rotateActivePlayer: true }, rounds, startingPhaseId: "select_artist", startingPlayer: "random" },
    variables: [], choices: [], clues: [], reveals: [], orderingItems: [], matchingItems: [], boards: [], resources: [], randomizers: [], media: [],
    decks: [{ id: "prompts", name: "Drawing prompts", visibility: "private", shuffle: true, initialHandSize: 0, cards: pack.prompts.map((prompt) => ({ id: prompt.id, title: prompt.prompt, body: `${prompt.category} · ${prompt.difficulty}`, icon: "✎", tags: [prompt.category, prompt.difficulty] })) }],
    components: [
      { id: "game_header", kind: "header", audience: "public", eyebrow: "BoardForge Original", title: { kind: "literal", value: "DrawBattle" }, description: { kind: "literal", value: "One secret prompt. One live canvas. Everyone else races to guess." }, icon: "✎" },
      { id: "prompt_deck", kind: "deck", audience: "active_player", deckId: "prompts", label: "Secret prompt" },
      { id: "secret_prompt", kind: "prompt", audience: "active_player", category: "Draw this", prompt: { kind: "active_card", deckId: "prompts", field: "title" }, hint: { kind: "active_card", deckId: "prompts", field: "body" }, icon: "✎" },
      { id: "live_canvas", kind: "drawing", audience: "public", label: "Live canvas" },
      { id: "guess_input", kind: "text_input", audience: "public", label: "Name the drawing", placeholder: "Type your guess…", multiline: false, maxLength: 48 },
      ...
      { id: "outcome", kind: "outcome", audience: "public", title: { kind: "literal", value: "Final gallery" }, description: { kind: "literal", value: "The team with the most correct guesses wins DrawBattle." } },
    ],
    actions: [
      { id: "select_artist", label: "Choose the artist", kind: "select_player", actor: "team_captain", oncePerPhase: true, effects: [{ kind: "set_active_player", mode: "selected" }, { kind: "advance_phase" }] },
      { id: "draw_prompt", label: "Reveal my prompt", kind: "draw", actor: "active_player", oncePerPhase: true, deckId: "prompts", effects: [{ kind: "draw_cards", deckId: "prompts", count: 1, target: "actor" }, { kind: "advance_phase" }] },
      { id: "pass_prompt", label: "Pass prompt", kind: "advance", actor: "active_player", oncePerPhase: true, effects: resolveEffects },
      { id: "submit_guess", label: "Submit guess", kind: "text", actor: "guessers", oncePerPhase: false, answerDeckId: "prompts", effects: [] },
    ...
    rules: [
      ...
      { id: "end_after_pass", trigger: { kind: "after_action", actionId: "pass_prompt" }, conditionMode: "all", conditions: [{ kind: "round_at_least", round: rounds + 1 }], effects: [{ kind: "end_game", winnerBy: "highest_score" }] },
    ],
    phases: [
      { id: "select_artist", title: "The captain chooses", componentIds: ["game_header", "round", "turn", "teams", "scores"], actionIds: ["select_artist"], nextPhaseId: "draw_prompt", completionMode: "manual", completionConditions: [], onComplete: [] },
      { id: "draw_prompt", title: "Secret prompt", componentIds: ["game_header", "round", "turn", "teams", "scores", "prompt_deck"], actionIds: ["draw_prompt"], nextPhaseId: "drawing", completionMode: "manual", completionConditions: [], onComplete: [] },
      { id: "drawing", title: "Live canvas", componentIds: ["game_header", "round", "turn", "teams", "scores", "draw_timer", "secret_prompt", "live_canvas", "guess_input"], actionIds: ["pass_prompt", "submit_guess", "draw_stroke"], nextPhaseId: "select_artist", completionMode: "manual", completionConditions: [], onComplete: [] },
    ],
  };
  const validation = validateComposedGameSpec(spec);
  if (!validation.ok) throw new Error(`Invalid DrawBattle GameSpec: ${validation.issues.map((issue) => `${issue.code}:${issue.path}`).join(", ")}`);
  return validation.spec;
}

export const defaultDrawBattlePack = createRandomDrawBattlePack({ themeId: "arcade", promptCount: 18 }, "boardforge-default-draw-battle");
