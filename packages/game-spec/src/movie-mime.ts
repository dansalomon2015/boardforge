...
export function createRandomMovieMimePack(setupInput: MovieMimeSetupInput, seed: string): MimeFilmPack {
  const setup = movieMimeSetupSchema.parse(setupInput);
...
export function createMovieMimeSpec(packInput: MimeFilmPack): ComposedGameSpec {
  ...
  const suffix = numberHash(
    `captain-loop-v2-en:${pack.themeId}:${pack.films.map((film) => film.id).join(":")}`,
  ).toString(36);
  const spec: ComposedGameSpec = {
    ...
    components: [
      {
        ...
        title: { kind: "literal", value: "CineMimes" },
        description: { kind: "literal", value: "One secret movie, one performer, one team racing to name it." },
        icon: "🎬",
      },
      { id: "film_deck", kind: "deck", audience: "active_player", deckId: "films", label: "Secret movie" },
      {
        ...
        category: "Act it out without speaking",
        prompt: { kind: "active_card", deckId: "films", field: "title" },
        hint: { kind: "active_card", deckId: "films", field: "body" },
        icon: "🎭",
      },
      ...
      { id: "scores", kind: "scores", audience: "public", title: "Team box office" },
      {
        ...
        kind: "outcome",
        audience: "public",
        title: { kind: "literal", value: "That is a wrap!" },
        description: { kind: "literal", value: "The team with the most correct movies wins." },
      },
    ],
    actions: [
      {
      ...
      {
        id: "draw_film",
        label: "Reveal my movie",
        kind: "draw",
        actor: "active_player",
        oncePerPhase: true,
        deckId: "films",
        effects: [{ kind: "draw_cards", deckId: "films", count: 1, target: "actor" }, { kind: "advance_phase" }],
      },
      {
        id: "film_guessed",
        label: "Movie guessed",
        ...
        oncePerPhase: true,
        effects: [
          { kind: "add_score", target: "actor_team", amount: 1 },
          { kind: "discard_selected_card", deckId: "films", target: "actor" },
          { kind: "advance_round", resetPhaseActions: true },
          { kind: "set_active_player", mode: "next_team_captain" },
          { kind: "advance_phase" },
        ],
      },
      {
    ...
    rules: [
      {
        id: "end_after_success",
        trigger: { kind: "after_action", actionId: "film_guessed" },
        conditionMode: "all",
      ...
      {
        id: "end_after_pass",
        trigger: { kind: "after_action", actionId: "film_passed" },
        conditionMode: "all",
        conditions: [{ kind: "round_at_least", round: rounds + 1 }],
        effects: [{ kind: "end_game", winnerBy: "highest_score" }],
      },
    ...
    phases: [
      ...
      },
      {
        ...
        componentIds: ["game_header", "round", "turn", "teams", "scores", "film_deck"],
        actionIds: ["draw_film"],
        nextPhaseId: "mime",
        completionMode: "manual",
        completionConditions: [],
      ...
      {
        id: "mime",
        title: "Lights, camera, mime!",
        componentIds: ["game_header", "round", "turn", "teams", "scores", "mime_timer", "film_prompt"],
        actionIds: ["film_guessed", "film_passed"],
        nextPhaseId: "select_mimer",
        completionMode: "manual",
        completionConditions: [],
        onComplete: [],
      },
  ...
  if (!validation.ok) {
    throw new Error(
      `Invalid movie mime GameSpec: ${validation.issues.map((issue) => `${issue.code}:${issue.path}`).join(", ")}`,
    );
  }
