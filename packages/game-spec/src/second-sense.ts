...
export const secondSenseSetupSchema = z.object({
...
export function createSecondSenseSpec(setupInput: SecondSenseSetupInput): ComposedGameSpec {
  ...
  const spec: ComposedGameSpec = {
    ...
    template: "composed",
    title: "Second Sense",
    description: "Feel the target time without a clock, stop on instinct, and survive the cut until only one player remains.",
    theme,
    minPlayers: 2,
    maxPlayers: 12,
    ...
    components: [
      { id: "game_header", kind: "header", audience: "public", eyebrow: "BoardForge Original", title: { kind: "literal", value: "Second Sense" }, description: { kind: "literal", value: "No countdown. No second chance. Just your sense of time." }, icon: "◉" },
      { id: "sense_board", kind: "second_sense", audience: "public", minTargetMs: range.minTargetMs, maxTargetMs: range.maxTargetMs, precisionMs: 10 },
      { id: "players", kind: "players", audience: "public" },
      { id: "outcome", kind: "outcome", audience: "public", title: { kind: "literal", value: "Time champion" }, description: { kind: "literal", value: "The final pulse belongs to one player." } },
    ],
    actions: [
      { id: "start_clock", label: "Start", kind: "timing_start", actor: "any_player", oncePerPhase: false, secondSenseId: "sense_board", effects: [{ kind: "start_timing", secondSenseId: "sense_board" }] },
      { id: "stop_clock", label: "Stop", kind: "timing_stop", actor: "any_player", oncePerPhase: false, secondSenseId: "sense_board", effects: [{ kind: "stop_timing", secondSenseId: "sense_board" }] },
      { id: "next_stage", label: "Next target", kind: "timing_advance", actor: "host", oncePerPhase: false, secondSenseId: "sense_board", effects: [{ kind: "advance_timing_round", secondSenseId: "sense_board" }] },
    ],
    rules: [],
    phases: [
      { id: "sense", title: "Trust your internal clock", componentIds: ["game_header", "sense_board", "players", "outcome"], actionIds: ["start_clock", "stop_clock", "next_stage"], completionMode: "manual", completionConditions: [], onComplete: [] },
    ],
  };
  const validation = validateComposedGameSpec(spec);
  if (!validation.ok) throw new Error(`Invalid Second Sense GameSpec: ${validation.issues.map((issue) => `${issue.code}:${issue.path}`).join(", ")}`);
