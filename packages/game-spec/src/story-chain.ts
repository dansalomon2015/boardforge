...
export const storyChainSetupSchema = z.object({
...
export const storyTwistSchema = z.object({
  ...
  requiredWord: z.string().trim().regex(/^[A-Za-z]+(?:[ '-][A-Za-z]+)?$/).min(2).max(28),
  direction: z.string().trim().min(8).max(160),
}).strict();

export const storyChainPackSchema = z.object({
...
const openings: Record<StoryMood, { title: string; opening: string }> = {
  chaotic: { title: "The Night Everything Went Sideways", opening: "At precisely 8:17, every light in the city turned purple and the mayor's goldfish announced that it knew why." },
  mystery: { title: "The Last Key at Bellweather House", opening: "The envelope had no stamp, no address, and only one sentence inside: return the key before the clock strikes thirteen." },
  fantasy: { title: "The Map That Remembered", opening: "On the morning the mountains began to move, a young mapmaker discovered a road that had never existed before." },
  spooky: { title: "Someone Knocked from Inside the Wall", opening: "The old house had been silent for fifty years, until three soft knocks answered from behind the freshly painted wall." },
  romantic: { title: "Meet Me Where the Trains Forget", opening: "They had missed each other by seven minutes every Friday for a year, until one impossible note appeared on both their tickets." },
  family: { title: "The Great Sunday Adventure", opening: "Grandma's famous pie vanished from the windowsill, leaving a trail of blue crumbs and one very suspicious garden gnome." },
};
...
export function createStoryChainPack(
  ...
  if (content.twists.length !== count) throw new Error(`StoryChain requires exactly ${count} twists for a ${setup.length} story.`);
  ...
  const leakedWord = content.twists.find((twist) => publicPremise.includes(` ${normalize(twist.requiredWord).trim()} `));
  if (leakedWord) throw new Error(`StoryChain required word “${leakedWord.requiredWord}” is already visible in the public premise.`);
...
export function createRandomStoryChainPack(setupInput: StoryChainSetupInput, seed: string): StoryChainPack {
  ...
  return createStoryChainPack(setup, { ...base, twists: deterministicShuffle(storyTwistCatalog, `${seed}:${setup.mood}`).slice(0, storyRounds[setup.length]) }, "random");
}
...
export function createStoryChainSpec(packInput: StoryChainPack): ComposedGameSpec {
  ...
  const suffix = numberHash(`story-chain-v1:${pack.title}:${pack.twists.map((twist) => twist.id).join(":")}`).toString(36);
  const resolveTurn = [
  ...
  const spec: ComposedGameSpec = {
    ...
    template: "composed",
    title: "StoryChain",
    description: pack.preferences ? `A shared story inspired by your idea: “${pack.preferences}”.` : "Build one unforgettable story together, one secret twist and one voice at a time.",
    theme,
    minPlayers: 2,
    maxPlayers: 12,
    ...
    decks: [{ id: "twists", name: "Secret twists", visibility: "private", shuffle: true, initialHandSize: 0, cards: pack.twists.map((twist) => ({ id: twist.id, title: twist.requiredWord, body: twist.direction, icon: "✦", tags: [pack.mood] })) }],
    components: [
      { id: "game_header", kind: "header", audience: "public", eyebrow: "BoardForge Original", title: { kind: "literal", value: pack.title }, description: { kind: "literal", value: "Write one or two sentences. Keep the story moving. Make your secret word feel inevitable." }, icon: "✦" },
      { id: "story", kind: "story", audience: "public", opening: { kind: "literal", value: pack.opening }, actionId: "continue_story", label: "Our story" },
      { id: "twist_deck", kind: "deck", audience: "active_player", deckId: "twists", label: "Your secret twist" },
      { id: "secret_twist", kind: "prompt", audience: "active_player", category: "Your sentence must include", prompt: { kind: "active_card", deckId: "twists", field: "title" }, hint: { kind: "active_card", deckId: "twists", field: "body" }, icon: "✦" },
      { id: "story_input", kind: "text_input", audience: "active_player", label: "Continue the story", placeholder: "Write one or two sentences…", multiline: true, maxLength: 320 },
      { id: "turn", kind: "turn", audience: "public" },
      { id: "round", kind: "round", audience: "public", label: "Chapter" },
      { id: "outcome", kind: "outcome", audience: "public", title: { kind: "literal", value: "The end — for now" }, description: { kind: "literal", value: "Read your one-of-a-kind story aloud, from the very first line to the final twist." } },
    ],
    actions: [
      { id: "draw_twist", label: "Reveal my secret twist", kind: "draw", actor: "active_player", oncePerPhase: true, deckId: "twists", effects: [{ kind: "draw_cards", deckId: "twists", count: 1, target: "actor" }, { kind: "advance_phase" }] },
      { id: "continue_story", label: "Add to the story", kind: "text", actor: "active_player", oncePerPhase: true, requiredWordDeckId: "twists", effects: resolveTurn },
    ],
    rules: [
      { id: "end_story", trigger: { kind: "after_action", actionId: "continue_story" }, conditionMode: "all", conditions: [{ kind: "round_at_least", round: rounds + 1 }], effects: [{ kind: "end_game", winnerBy: "none" }] },
    ],
    phases: [
      { id: "draw_twist", title: "A new twist", componentIds: ["game_header", "story", "round", "turn", "twist_deck", "outcome"], actionIds: ["draw_twist"], nextPhaseId: "write", completionMode: "manual", completionConditions: [], onComplete: [] },
      { id: "write", title: "Continue the tale", componentIds: ["game_header", "story", "round", "turn", "secret_twist", "story_input", "outcome"], actionIds: ["continue_story"], nextPhaseId: "draw_twist", completionMode: "manual", completionConditions: [], onComplete: [] },
    ],
  };
  const validation = validateComposedGameSpec(spec);
  if (!validation.ok) throw new Error(`Invalid StoryChain GameSpec: ${validation.issues.map((issue) => `${issue.code}:${issue.path}`).join(", ")}`);
  return validation.spec;
}

export const defaultStoryChainPack = createRandomStoryChainPack({ themeId: "cozy", mood: "chaotic", length: "full" }, "boardforge-default-story-chain");
