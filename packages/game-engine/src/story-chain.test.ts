...
describe("StoryChain release gate", () => {
  it("keeps the current twist private, enforces it and publishes accepted prose", () => {
    const spec = createStoryChainSpec(createRandomStoryChainPack({ themeId: "cozy", mood: "fantasy", length: "quick" }, "privacy"));
    let state = initializeComposedGame(spec, players, "story-room");
    const authorId = state.activePlayerId;
    state = reduceComposedGame(state, { type: "COMPOSED_ACTION", actionId: "draw_twist", idempotencyKey: "story-draw-001" }, authorId, authorId === "p1", spec);
    ...
    expect(JSON.stringify(projectComposedGameState(state, spec, players, "STORY1", readerId))).not.toContain(requiredWord);
    expect(() => reduceComposedGame(state, { type: "COMPOSED_ACTION", actionId: "continue_story", idempotencyKey: "story-write-bad", payload: { text: "Nothing unusual happened after all." } }, authorId, authorId === "p1", spec)).toThrow(`include the word`);

    const contribution = `Then the ${requiredWord} opened a door that had never been there before.`;
    state = reduceComposedGame(state, { type: "COMPOSED_ACTION", actionId: "continue_story", idempotencyKey: "story-write-good", payload: { text: contribution } }, authorId, authorId === "p1", spec);
    ...
    expect(story?.data.entries).toEqual([{ sequence: 2, round: 1, actorId: authorId, actorName: players.find((player) => player.id === authorId)!.name, text: contribution }]);
    expect(state.activePlayerId).not.toBe(authorId);
    expect(state.round).toBe(2);
  });

  it("completes every generated story in virtual-agent playtests", () => {
    const spec = createStoryChainSpec(createRandomStoryChainPack({ themeId: "enchanted", mood: "chaotic", length: "full" }, "release"));
