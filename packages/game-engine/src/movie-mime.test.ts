...
import { createMovieMimeSpec, createRandomMovieMimePack } from "@boardforge/game-spec";
...
describe("movie mime release gate", () => {
  ...
  it("lets the chosen captain select a teammate and rotates to the next team captain", () => {
    const spec = createMovieMimeSpec(createRandomMovieMimePack({ themeId: "noir", filmCount: 6 }, "captain-flow"));
    ...
    const selectedMimer = state.teams
      .find((team) => team.id === startingTeamId)!
      .playerIds.find((id) => id !== startingCaptain)!;

    expect(
      projectComposedGameState(state, spec, players, "MIME01", startingCaptain).availableActions.map(
        (action) => action.id,
      ),
    ).toContain("select_mimer");
